...
const SeriesDetail = () => {
    ...
    useEffect(() => {
        const loadData = async () => {
            ...
            try {
                ...
                // 5. Fetch Display Preferences for backdrop setting
                try {
    ...
    useEffect(() => {
        if (series) {
            document.title = `Legitflix - ${series.Name}`;
        } else {
            document.title = "LegitFlix - Series";
        }
    }, [series]);
    ...
    const playEpisode = async (episodeId) => {
        ...
        if (savedCast) {
            try {
                ...
                if (found) {
                    epName = found.Name;
                } else if (nextUpEpisode && nextUpEpisode.Id === episodeId) {
                    epName = nextUpEpisode.Name;
                }
                await jellyfinService.playOnSession(target.id, episodeId);
                alert(`Casting "${epName}" to ${target.name}`);
                return;
            } catch (err) {
    ...
    const handleEpisodeClick = (episodeId) => {
        if (isSelectionMode) {
            ...
            const isCurrentlySelected = selectedEpisodes.includes(episodeId);

            if (element) {
                // Bounce scale animation on card selection
                gsap.fromTo(element,
                    { scale: 0.96 },
                    {
                        scale: 1,
                ...
                // Animate checkmark box
                const checkbox = element.querySelector('.lf-episode-checkbox');
                if (checkbox) {
                    if (!isCurrentlySelected) {
                        gsap.fromTo(checkbox,
                            { scale: 0.6, rotation: -12 },
                            { scale: 1, rotation: 0, duration: 0.35, ease: "back.out(1.8)" }
                        );
                    } else {
                        gsap.to(checkbox, { scale: 0.8, duration: 0.2, ease: "power2.in" });
                    }
                }
            }
    ...
    const toggleFavorite = async () => {
        try {
            ...
            window.dispatchEvent(new CustomEvent('favoriteToggled', {
                detail: { itemId: series.Id, isFavorite: newFav }
            }));
            ...
            window.dispatchEvent(new CustomEvent('favoriteToggled', {
                detail: { itemId: series.Id, isFavorite: isFavorite }
            }));
        }
    };
    ...
    useEffect(() => {
        const handleClickOutside = (e) => {
            if (isActionMenuOpen && !e.target.closest('.lf-series-actions-dropdown-wrapper')) {
    ...
    const handleConfirmDelete = async () => {
        ...
        try {
            ...
            showToast(`${deleteItem.Type || 'Series'} deleted successfully.`, 'success');
            setDeleteItem(null);
            navigate('/');
        } catch (e) {
    ...
    const handleActionClick = async (actionId) => {
        ...
        switch (actionId) {
            ...
            case 'addtoplaylist':
                ...
                break;
            case 'downloadall':
                const downloadUrl = jellyfinService.getDownloadUrl(series.Id);
                window.open(downloadUrl, '_blank');
                showToast('Series download started.', 'success');
                break;
            case 'delete':
                setDeleteItem(series);
                break;
            case 'edit':
            ...
            case 'identify':
                ...
                break;
            case 'refresh':
                const success = await jellyfinService.refreshItem(series.Id);
                if (success) {
    ...
    // Cast processing: Take top 30
    const cast = series.People ? series.People.slice(0, 30) : [];
    ...
    return (
        <div className="lf-series-container">
            ...
            >
                <div className={`lf-series-hero__backdrop ${isTrailerPlaying ? 'is-hidden' : ''}`} style={enableBackdrops ? { backgroundImage: `url('${backdropUrl}')` } : { background: '#141414' }}></div>
                            ...
                            >
                                <div className="lf-episode-card__thumbnail">
                                    ...
                                    {isPlayed && !isSelectionMode && (
                                        <div className="lf-episode-checkbox" style={{ opacity: 1, background: '#00e676', borderColor: '#00e676' }}>
