...
const Favorites = () => {
    ...
    useEffect(() => {
        document.title = "LegitFlix - My Favorites";
    }, []);
    ...
    const handleMenuAction = async (action, item) => {
        ...
        switch (action) {
            ...
            case 'unfavorite': {
                const user = await jellyfinService.getCurrentUser();
                if (user) {
                    await jellyfinService.markFavorite(user.Id, item.Id, false);
    ...
    const getContextMenuOptions = (item) => {
        ...
        return [
            ...
            { label: 'Remove from Favorites', icon: 'favorite_border', action: () => handleMenuAction('unfavorite', item) },
            { type: 'separator' },
