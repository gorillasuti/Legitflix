...
const InfoModal = ({ itemId, onClose, isOpen }) => {
    ...
    useEffect(() => {
        const handleToggle = (e) => {
            const { itemId: targetId, isFavorite: newFav } = e.detail;
            if (details && details.Id === targetId) {
                setDetails(prev => ({
                    ...prev,
    ...
    const toggleFavorite = async () => {
        ...
        window.dispatchEvent(new CustomEvent('favoriteToggled', {
            detail: { itemId: details.Id, isFavorite: newFav }
        }));
        try {
                    ...
                    <div className="info-col-left">
                        <div className="info-meta-row">
                            ...
                            <span className="info-duration">
                                {details.Type === 'Series' ? (
                                    <>
                                        {(details.ChildCount || 0) > 0 ? `${details.ChildCount} Season${details.ChildCount === 1 ? '' : 's'}` : ''}
                                        {(details.ChildCount || 0) > 0 && (details.RecursiveItemCount || 0) > 0 ? ' • ' : ''}
                                        {(details.RecursiveItemCount || 0) > 0 ? `${details.RecursiveItemCount} Episode${details.RecursiveItemCount === 1 ? '' : 's'}` : ''}
                                    </>
                                ) : (
                                    details.RunTimeTicks ? Math.round(details.RunTimeTicks / 600000000) + 'm' : ''
                                )}
                            </span>
                        ...
                        {trailers.map(t => (
                            <div key={t.id} className="trailer-card" onClick={() => setActiveTrailerId(t.id)}>
                                <div className="trailer-thumb">
                                    ...
                                    <div className="play-overlay"><span className="material-icons">play_arrow</span></div>
                                </div>
